...
public:
    Hashtable(int size,int sRandInit,int k,int w,int r,int d,int DatasetSize);
...
Hashtable::Hashtable(int size,int sRandInit,int k,int w,int r,int d,int DatasetSize)
{
...
int Hashtable::hash_function(vector<double> &image,int testing){
    ...
    return final_hash % this->datasetSize/16;  //Return to teliko result tis hash function
}
