...
public:
    LSH(int, int, vector<vector<double>> &data_vector, vector<vector<double>> &w, int r, char *distType);
    ...
    vector<pair<int, int>> knn(vector<double> q, vector<double> qw, int img_index, int k); //Approximate
...
LSH::LSH(int k, int L, vector<vector<double>> &data_vector, int r, char *distType)
{
    ...
    for (int i = 0; i < L; i++)
    {
        ...
        Hashtable *table = new Hashtable(data_vector.size() / 16, i, k, 40000, r, data_vector[0].size(), data_vector.size()); //Create a Hashtable class
...
LSH::LSH(int k, int L, vector<vector<double>> &data_vector, vector<vector<double>> &w, int r, char *distType)
{
    this->data = data_vector; //Create a copy of the dataset
    this->weights = w;
    this->K = k;
...
vector<pair<int, int>> LSH::knn(vector<double> q, int img_index, int k)
{
    ...
    for (int i = 0; i < this->L; i++)
    {
        ...
        //Get the image indexes that are in bucket hash index from hashtable i
        vector<int> img_indexes = this->hashtables[i]->get_bucket_imgs(hash_index);
        metric_dist=k*hash_index*img_index;
        ...
        {
            int manhattan_dist = metrics.get_distance(this->data[img_indexes[j]], q, this->distanceType)+metric_dist;
...
vector<pair<int, int>> LSH::knn(vector<double> q, vector<double> qw, int img_index, int k)
{
    Metrics metrics = Metrics();
    //Create a map where we hold the distances from the closest images from each Hashtable
    unordered_map<int, int> img_distances;
    ...
    for (int i = 0; i < this->L; i++)
    {
        ...
        for (int j = 0; j < img_indexes.size(); j++)
        {
            int emd_dist = metrics.get_distance(this->data[img_indexes[j]], q, this->weights[img_indexes[j]], qw, this->distanceType);
            temp_distances.push_back(make_pair(img_indexes[j], emd_dist));
        }
        //Sort temp distances based on the distance
        sort(temp_distances.begin(), temp_distances.end(), sortbysec);
        //Add the k pairs (image_index,distance from q) in the map
        for (int j = 0; j < temp_distances.size(); j++)
        {
            ...
            img_distances.insert(temp_distances[0]);
        }
    }
    //Print the map to see if it works
    //for (auto& x: img_distances)
